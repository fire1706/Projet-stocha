...
S_zz = zeros(length(U),length(omega_1)); S_tt = zeros(length(U),length(omega_1));
sigma_zz = zeros; sigma_tt = zeros;
j = 1;
while j <= length(U)
    ...
    while i <= length(omega_1)
        ...
        [S_zz(j,i),S_tt(j,i)] = PSD_omega_1(omega_1(i),U(j),B,rho,H(:,:,i),L_w,I_w);
        
        i = i + 1;
    end
    sigma_zz(j) = sqrt(trapz(omega_1,S_zz(j,:))); % Ecart-type de la PSD_zz 
    sigma_tt(j) = sqrt(trapz(omega_1,S_tt(j,:))); % Ecart-type de la PSD_thetatheta
